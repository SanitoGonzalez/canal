...
/// This crate has a runtime dependency on artifacts produced by the `canal-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `canal-ebpf` would cause build (and
...
/// performance; an environment variable is used to replace true builds of `canal-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `canal-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "canal-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `canal-ebpf`, so cargo will automatically rebuild us
        // if `canal-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `canal-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("canal-ebpf");
